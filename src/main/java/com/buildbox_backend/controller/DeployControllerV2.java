...
public class DeployControllerV2 {
    ...
    @PostMapping("/v2")
    public ResponseEntity<String> deployV2(@RequestBody GitCloneRequest request) {
        System.out.println("Starting deployment for: " + request.getLink());

        System.out.println(request.getUserId());

        ecsService.startBuild(request.getLink(), request.getProjectName(), request.getUserId(), request.getBackendDirectory(), request.getFrontendDirectory());
